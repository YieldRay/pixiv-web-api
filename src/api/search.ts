...
export const rpcCps = (query: Query<{ keyword: string }>) =>
    requestJSON<{
        candidates: Array<{ access_count: string; tag_name: string; tag_translation: string; type: string }>;
    }>(`/rpc/cps.php`, query);
 ...
 *
 */
export const searchSuggestion = () => requestJSONAPI(`/ajax/search/suggestion?mode=all`);
