 ...
 * 奇奇怪怪的接口
 */
export const userWorksLatest = (id: number, query?: Query) => requestJSONAPI(`/ajax/user/${id}/works/latest`, query);
...
export const userExtra = (query?: Query) =>
    requestJSONAPI<{ background: string | null; followers: number; following: number; mypixivCount: number }>(
        `/ajax/user/extra`,
        query
    );
...
export const rpc = (
    data: Query<
        | { mode: "following_user_restrict_change"; user_id: string; restrict: 0 | 1 }
        | {
